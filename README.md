...
# This software is provided "as is," without warranty of any kind. The author shall not be liable for any damages arising from its use.
...
- **Checksums:** 
  - Generate and verify checksums for files and directories
- **Sidecar manifests:**
...
- **Performance:** 
  - Parallel hashing (`-p N`) with adaptive batching (`-b RULES`)
- **Debug helpers:**
...
| Type        | Install Command(s) | Uninstall Command(s) |
|-------------|--------------------|-----------------------|
| User        | `make user-install` or `./scripts/install.sh` | `make user-uninstall` or `./scripts/uninstall.sh` |
| Developer   | `make install`     | `make uninstall`      |

Reinstall with new version:
`make user-reinstall` or
`./scripts/uninstall.sh` and then
`./scripts/install.sh`
...
- Initializes `.meta`/`.log` from existing `.md5`  
- Overwrites mismatched manifests automatically  
- Produces detailed first-run log  

#### Verify-only Audit
`checksums -V -a md5 -m sha256 -p 4 --md5-details --skip-empty --assume-yes /data/project`  
- Audits integrity without writing sidecars  
- Reports `VERIFIED`, `MISMATCH`, or `MISSING`  

#### Dry-run Planning
`checksums -n -a sha256 -m sha256 -p 4 -v --md5-details --skip-empty --assume-yes /data/project`  
- Simulates actions without writes  
- Logs “would hash” messages for preview
