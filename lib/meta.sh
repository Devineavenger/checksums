...
read_meta() {
  ...
  while IFS=$'\t' read -r path inode dev mtime size hash; do
    [ -z "$path" ] && continue
    case "$path" in \#meta|\#sig) continue ;; esac
...
verify_meta_sig() {
  ...
  tmp=$(mktemp) || return 2
  awk '!/^#sig\b/' "$meta" >"$tmp"
  if [ "$META_SIG_ALGO" = "sha256" ] && command -v sha256sum >/dev/null 2>&1; then
    expected=$(sha256sum <"$tmp" | awk '{print $1}')
  elif [ "$META_SIG_ALGO" = "sha256" ]; then
    expected=$(shasum -a 256 <"$tmp" | awk '{print $1}')
  elif command -v md5sum >/dev/null 2>&1; then
    expected=$(md5sum <"$tmp" | awk '{print $1}')
  else
    expected=$(md5 <"$tmp" 2>/dev/null | awk '{print $1}')
  fi
  rm -f "$tmp"
  [ "$expected" = "$stored" ]
}
...
write_meta() {
  ...
  if [ "$META_SIG_ALGO" != "none" ] && [ -s "$tmp" ]; then
    if [ "$META_SIG_ALGO" = "sha256" ] && command -v sha256sum >/dev/null 2>&1; then
      sig=$(sha256sum <"$tmp" | awk '{print $1}')
    elif [ "$META_SIG_ALGO" = "sha256" ]; then
      sig=$(shasum -a 256 <"$tmp" | awk '{print $1}')
    elif command -v md5sum >/dev/null 2>&1; then
      sig=$(md5sum <"$tmp" | awk '{print $1}')
    else
      sig=$(md5 <"$tmp" 2>/dev/null | awk '{print $1}')
    fi
    printf '#sig\t%s\n' "$sig" >>"$tmp"
  fi
