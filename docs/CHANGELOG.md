...
### Tests
* test: new `tests/test_menu.bats` — 55 tests covering sourcing sanity, TTY guard, 8 validators (algo, meta-sig, dir, file, parallel, size, log-format), 23 command construction scenarios (default/non-default flags, all modes, quoting, mutual exclusivity), flag parsing integration (`--menu`/`--interactive` sets MENU_MODE, no TARGET_DIR required, pre-set globals preserved), help text coverage, and end-to-end scripted flow with `_MENU_FORCE_TTY` escape hatch; 1 new sourcing-sanity test in `test_usage.bats`
...
### CI/CD
* ci: add macOS runner to CI matrix — `ci.yml` now runs on both `ubuntu-latest` and `macos-latest` with `fail-fast: false`; macOS job installs Bash 5.x, GNU coreutils, bats-core, shellcheck, jq, and parallel via Homebrew; `GITHUB_PATH` updated to prefer Homebrew binaries over BSD system tools; PR changelog comment restricted to Linux job to avoid duplicates; validates cross-platform tool detection (BSD stat, md5 -r, shasum fallbacks in `tools.sh` and `stat.sh`)
